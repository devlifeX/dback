...
**Current app version in repo:** `3.8.0` → About screen and local `./build.sh` use this until you bump again.
...
Outputs: `dist/dback-linux`, `dist/dback`, `dist/dback_3.8.0_amd64.deb`.  
`build.sh` prints the **release git tag** to push when the build succeeds.

### GitHub Release (after merging to `master`)

Tag **must** match `main.go` / `build.sh` version (`3.8.0` → tag `v3.8.0`). CI strips the `v` and embeds the version in binaries and the `.deb` name.
...
| Debian package | `dback_3.8.0_amd64.deb` |
...
4. Tell the user the **release git tag** to push: `v{same version}` (e.g. **`v3.8.0`** for app version `3.8.0`).
...
CI reads the tag (`v3.8.0` → `APP_VERSION=3.8.0`); tag and `main.go`/`build.sh` must always match. PPA changelog per Ubuntu series is synced in CI — do not commit jammy/noble-specific changelog entries unless doing a manual PPA upload.
...
When this doc and code diverge, **trust the code** and update this file. Last aligned with v3.8.0 — Go 1.22 toolchain, jammy/noble PPA matrix, offline vendor builds, in-app updater, Dry-Run Verify (SHA256 + fingerprint + deep verify), and mandatory app version bumps on changes.
