...
**Current app version in repo:** `3.7.1` → About screen and local `./build.sh` use this until you bump again.
...
./build.sh linux
# or explicitly:
APP_VERSION=3.7.1 ./build.sh linux
```

Outputs: `dist/dback-linux`, `dist/dback`, `dist/dback_3.7.1_amd64.deb`.  
`build.sh` prints the **release git tag** to push when the build succeeds.

### GitHub Release (after merging to `master`)

Tag **must** match `main.go` / `build.sh` version (`3.7.1` → tag `v3.7.1`). CI strips the `v` and embeds the version in binaries and the `.deb` name.
...
| Debian package | `dback_3.7.1_amd64.deb` |
...
1. **Bump the app version** (patch by default: `3.7.1` → `3.7.2`; minor: `3.7.1` → `3.8.0`):
...
4. Tell the user the **release git tag** to push: `v{same version}` (e.g. **`v3.7.1`** for app version `3.7.1`).
...
CI reads the tag (`v3.7.1` → `APP_VERSION=3.7.1`); tag and `main.go`/`build.sh` must always match. PPA changelog per Ubuntu series is synced in CI — do not commit jammy/noble-specific changelog entries unless doing a manual PPA upload.
...
|-------------|--------------|
| Bug fix, CI/packaging, docs tied to app behavior | `3.7.1` → `3.7.2` |
