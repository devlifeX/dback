...
**Current app version in repo:** `3.6.9` → About screen and local `./build.sh` use this until you bump again.
...
./build.sh linux
# or explicitly:
APP_VERSION=3.6.9 ./build.sh linux
```

Outputs: `dist/dback-linux`, `dist/dback`, `dist/dback_3.6.9_amd64.deb`.  
`build.sh` prints the **release git tag** to push when the build succeeds.

### GitHub Release (after merging to `master`)

Tag **must** match `main.go` / `build.sh` version (`3.6.9` → tag `v3.6.9`). CI strips the `v` and embeds the version in binaries and the `.deb` name.
...
| Debian package | `dback_3.6.9_amd64.deb` |
...
1. **Bump the app version** (patch by default: `3.6.9` → `3.6.10`):
...
4. Tell the user the **release git tag** to push: `v{same version}` (e.g. **`v3.6.9`** for app version `3.6.9`).
...
CI reads the tag (`v3.6.9` → `APP_VERSION=3.6.9`); tag and `main.go`/`build.sh` must always match. PPA changelog per Ubuntu series is synced in CI — do not commit jammy/noble-specific changelog entries unless doing a manual PPA upload.
...
|-------------|--------------|
| Bug fix, CI/packaging, docs tied to app behavior | `3.6.9` → `3.6.10` |
...
When this doc and code diverge, **trust the code** and update this file. Last aligned with v3.6.9 — Go 1.22 toolchain, jammy/noble PPA matrix, offline vendor builds, in-app updater, and mandatory app version bumps on changes.
