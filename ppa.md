...
| `debian/changelog` | خط اول، مثلاً `dback (3.8.1-1~ubuntu24.04.1) noble` |
...
```bash
# noble (24.04)
PPA_DIST=noble APP_VERSION=3.8.1 ./packaging/sync-debian-changelog.sh
# → dback (3.8.1-1~ubuntu24.04.1) noble

# jammy (22.04) — upload جدا با نسخه متفاوت
PPA_DIST=jammy APP_VERSION=3.8.1 ./packaging/sync-debian-changelog.sh
# → dback (3.8.1-1~ubuntu22.04.1) jammy
```
...
```
../dback_3.8.1-1~ubuntu24.04.1.dsc
../dback_3.8.1-1~ubuntu24.04.1.tar.xz
../dback_3.8.1-1~ubuntu24.04.1_source.changes
```
...
dput ppa:devlifex/dback ../dback_3.8.1-1~ubuntu24.04.1_source.changes
```

### upload دستی بعداً

```bash
debsign -k E8B25AD68688EC024359E03FE00C906928B7599C ../dback_3.8.1-1~ubuntu24.04.1_source.changes
dput -f ppa:devlifex/dback ../dback_3.8.1-1~ubuntu24.04.1_source.changes
```
...
|--------|--------------|----------------|
| noble (24.04) | `noble` | `3.8.1-1~ubuntu24.04.1` |
| jammy (22.04) | `jammy` | `3.8.1-1~ubuntu22.04.1` |
...
```bash
dput dback-ppa ../dback_3.8.1-1~ubuntu24.04.1_source.changes
```
...
```bash
PPA_DIST=noble APP_VERSION=3.8.1 ./packaging/sync-debian-changelog.sh
# یا: dch -v 3.8.1-1~ubuntu24.04.1 -D noble "Fix PPA target series."
```
...
git commit -m "Release 3.8.1"
git tag v3.8.1
git push origin master v3.8.1
```
...
## release فعلی — 3.8.1 (آماده build و PPA)

نسخه در `main.go`، `build.sh` و `debian/changelog` (noble) روی **3.8.1** است.

```bash
# بیلد محلی
./build.sh linux

# PPA دستی (هر series جدا):
export GPG_TTY=$(tty)
PPA_DIST=noble APP_VERSION=3.8.1 ./packaging/sync-debian-changelog.sh
UPLOAD=1 ./packaging/build-ppa.sh
...
## release بعدی — مثال (patch)

```bash
# 1. نسخه را در main.go و build.sh به 3.8.2 تغییر بده
git add main.go build.sh
git commit -m "Release 3.8.2"
git tag v3.8.2
git push origin master v3.8.2   # CI خودکار PPA + GitHub Release
```
