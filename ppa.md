...
| `debian/changelog` | خط اول، مثلاً `dback (3.8.0-1~ubuntu24.04.1) noble` |
...
```
../dback_3.8.0-1~ubuntu24.04.1.dsc
../dback_3.8.0-1~ubuntu24.04.1.tar.xz
../dback_3.8.0-1~ubuntu24.04.1_source.changes
```
...
dput ppa:devlifex/dback ../dback_3.8.0-1~ubuntu24.04.1_source.changes
```

### upload دستی بعداً

```bash
debsign -k E8B25AD68688EC024359E03FE00C906928B7599C ../dback_3.8.0-1~ubuntu24.04.1_source.changes
dput -f ppa:devlifex/dback ../dback_3.8.0-1~ubuntu24.04.1_source.changes
```
...
|--------|--------------|----------------|
| noble (24.04) | `noble` | `3.8.0-1~ubuntu24.04.1` |
| jammy (22.04) | `jammy` | `3.8.0-1~ubuntu22.04.1` |
...
```bash
dput dback-ppa ../dback_3.8.0-1~ubuntu24.04.1_source.changes
```
...
```bash
PPA_DIST=noble APP_VERSION=3.8.0 ./packaging/sync-debian-changelog.sh
# یا: dch -v 3.8.0-1~ubuntu24.04.1 -D noble "Fix PPA target series."
```
...
## release فعلی — 3.8.0 (آماده build و PPA)

نسخه در `main.go`، `build.sh` و `debian/changelog` (noble) روی **3.8.0** است.
...
```bash
# 1. نسخه را در main.go و build.sh به 3.8.1 تغییر بده
git add main.go build.sh
git commit -m "Release 3.8.1"
git tag v3.8.1
git push origin master v3.8.1   # CI خودکار PPA + GitHub Release
```
