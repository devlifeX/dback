...
```powershell
go build -ldflags "-X main.appVersion=3.6.4" -o dist/dback-windows.exe .
```
...
```bash
go build -ldflags "-X main.appVersion=3.6.4" -o dist/dback-linux .
```
...
```bash
sudo apt install ./dback_3.6.4_amd64.deb
```
